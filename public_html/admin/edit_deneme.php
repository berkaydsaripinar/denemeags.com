...
// Bu dosya, Süper Admin'in deneme sınavlarını eklemesini ve düzenlemesini sağlar.
...
$page_title = $is_editing ? "Deneme Sınavını Düzenle" : "Yeni Deneme Sınavı Ekle";
...
if ($is_editing) {
    try {
        ...
        if ($deneme_data_db) {
            ...
            set_admin_flash_message('error', 'Düzenlenecek deneme bulunamadı.');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_admin_csrf_token($_POST['csrf_token'])) {
        ...
        $errors = [];
        if (empty($deneme_data_post['deneme_adi'])) { $errors[] = "Deneme adı boş bırakılamaz."; }
        
        if (empty($sonuc_aciklama_tarihi_input)) {
            $errors[] = "Sıralama açıklanma tarihi boş bırakılamaz.";
        } else {
            try {
        ...
        if (empty($errors)) {
            try {
                if ($is_editing) {
                    $sql = "UPDATE denemeler SET deneme_adi = :deneme_adi, kisa_aciklama = :kisa_aciklama, soru_sayisi = :soru_sayisi, 
                    ...
                    $sql = "INSERT INTO denemeler (deneme_adi, kisa_aciklama, soru_sayisi, sonuc_aciklama_tarihi, cozum_linki, soru_kitapcik_dosyasi, resim_url, shopier_link, shopier_product_id, aktif_mi, anasayfada_goster) 
                            VALUES (:deneme_adi, :kisa_aciklama, :soru_sayisi, :sonuc_aciklama_tarihi, :cozum_linki, :soru_kitapcik_dosyasi, :resim_url, :shopier_link, :shopier_product_id, :aktif_mi, :anasayfada_goster)";
                     $params = $deneme_data_post;
                }
                $stmt = $pdo->prepare($sql);
                $stmt->execute($params);

                set_admin_flash_message('success', $is_editing ? 'Deneme başarıyla güncellendi.' : 'Deneme başarıyla eklendi.');
                header("Location: manage_denemeler.php"); exit;
            } catch (PDOException $e) {
        ...
        // Hata durumunda formu tekrar doldurmak için POST verilerini $deneme_data'ya geri yükle
...
<form action="edit_deneme.php<?php echo $is_editing ? '?id='.$deneme_id : ''; ?>" method="POST" style="max-width: 700px;">
    <input type="hidden" name="csrf_token" value="<?php echo $csrf_token; ?>">
    
    <div class="form-group mb-3">
        <label for="deneme_adi" class="form-label fw-bold">Deneme Adı:</label>
    ...
    <div class="form-group mb-3">
        ...
        <input type="number" id="soru_sayisi" name="soru_sayisi" class="input-admin form-control" 
               value="<?php echo escape_html($deneme_data['soru_sayisi']); ?>" required min="1">
    </div>
    <div class="form-group mb-3">
        <label for="sonuc_aciklama_tarihi" class="form-label">Sıralama Açıklanma Tarihi ve Saati:</label>
        <input type="datetime-local" id="sonuc_aciklama_tarihi" name="sonuc_aciklama_tarihi" class="input-admin form-control" 
               value="<?php echo escape_html($deneme_data['sonuc_aciklama_tarihi_formatted']); ?>" required>
    </div>
    ...
    <div class="card card-theme mb-3">
        ...
        <div class="card-body">
            <div class="form-group mb-3">
                ...
                <small class="form-text text-muted">Satış sonrası müşteriye e-posta ile otomatik gönderilecek olan PDF'in dosya adı. Bu dosyayı FTP ile <code>uploads/questions/</code> klasörüne yüklemelisiniz.</small>
            </div>
             <div class="form-group mb-0">
                ...
                <small class="form-text text-muted">Kullanıcının sınavı tamamladıktan sonra göreceği çözüm kitapçığı PDF'inin dosya adı. Bu dosyayı FTP ile <code>uploads/solutions/</code> klasörüne yüklemelisiniz.</small>
            </div>
        </div>
    </div>
    ...
    <div class="card card-theme mb-3">
        ...
        <div class="card-body">
            <div class="form-group mb-3">
                <label for="resim_url" class="form-label">Deneme Resmi URL'si (Anasayfa için):</label>
            ...
            <div class="form-group mb-3">
                <label for="shopier_link" class="form-label">Shopier Satış Linki (Anasayfa için):</label>
            ...
            <div class="form-group mb-3">
                ...
                <small class="form-text text-muted">Otomatik teslimat için Shopier panelinizdeki ürünün sayısal ID'sini buraya girin.</small>
            </div>
            <div class="form-check form-switch">
                ...
                <label class="form-check-label" for="anasayfada_goster_checkbox">Bu denemeyi anasayfada (giriş yapmadan önce) göster</label>
            </div>
        </div>
    </div>
    ...
    <div class="form-check form-switch mt-4 mb-3">
        ...
        <label class="form-check-label" for="aktif_mi_checkbox">Deneme Aktif Mi? (Kullanıcılar erişebilsin ve çözebilsin mi?)</label>
    </div>
