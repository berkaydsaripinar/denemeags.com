...
function normalize_storage_filename($value) {
    if (!is_string($value)) {
        return null;
    }
...
function resolve_existing_file(array $paths) {
...
function resolve_product_file_path($fileName) {
    $fileName = normalize_storage_filename($fileName);
    if ($fileName === null) {
        return null;
    }

    return resolve_existing_file([
        PRODUCT_UPLOADS_DIR . '/' . $fileName,
        PUBLIC_PRODUCT_UPLOADS_DIR . '/' . $fileName,
        PUBLIC_UPLOADS_DIR . '/' . $fileName,
    ]);
}

function resolve_video_file_path($fileName) {
    $fileName = normalize_storage_filename($fileName);
    if ($fileName === null) {
        return null;
    }
    ...
    return resolve_existing_file([
        VIDEO_UPLOADS_DIR . '/' . $fileName,
        PRODUCT_UPLOADS_DIR . '/' . $fileName,
        PUBLIC_PRODUCT_UPLOADS_DIR . '/' . $fileName,
    ]);
}

function resolve_document_file_path($fileName, $type = 'product') {
    $fileName = normalize_storage_filename($fileName);
    if ($fileName === null) {
        return null;
    }
    ...
    switch ($type) {
        case 'question':
            $candidates[] = QUESTIONS_UPLOADS_DIR . '/' . $fileName;
            break;
        case 'solution':
            $candidates[] = SOLUTIONS_UPLOADS_DIR . '/' . $fileName;
            break;
        case 'demo':
            $candidates[] = PUBLIC_PRODUCT_UPLOADS_DIR . '/' . $fileName;
            $candidates[] = QUESTIONS_UPLOADS_DIR . '/' . $fileName;
            break;
    }

    $candidates[] = PRODUCT_UPLOADS_DIR . '/' . $fileName;
    $candidates[] = PUBLIC_PRODUCT_UPLOADS_DIR . '/' . $fileName;
    $candidates[] = PUBLIC_UPLOADS_DIR . '/' . $fileName;

    return resolve_existing_file($candidates);
}

function get_document_preview_url($fileName, $type = 'question') {
    $fileName = normalize_storage_filename($fileName);
    if ($fileName === null) {
        return null;
    }
    ...
    switch ($type) {
        case 'question':
            $publicMappings[] = [QUESTIONS_UPLOADS_DIR . '/' . $fileName, 'uploads/questions/' . $fileName];
            break;
        case 'solution':
            $publicMappings[] = [SOLUTIONS_UPLOADS_DIR . '/' . $fileName, 'uploads/solutions/' . $fileName];
            break;
        case 'product':
        case 'image':
            $publicMappings[] = [PUBLIC_PRODUCT_UPLOADS_DIR . '/' . $fileName, 'uploads/products/' . $fileName];
            break;
    }

    foreach ($publicMappings as [$path, $relativeUrl]) {
        if (is_file($path)) {
...
function send_custom_mail($to, $subject, $content_title, $content_body, $button_text = '', $button_url = '') {
    $site_name = defined('SITE_NAME') ? SITE_NAME : 'Deneme AGS';
    ...
    $headers .= "From: $site_name <" . MAIL_FROM_ADDRESS . ">" . "\r\n";
...
function get_image_url($url) {
    ...
    if ($trimmed !== '' && $trimmed[0] === '/') {
        return escape_html(app_url(ltrim($trimmed, '/')));
    }

    $publicImageUrl = get_document_preview_url($trimmed, 'image');
    if ($publicImageUrl !== null) {
        return escape_html($publicImageUrl);
    }
