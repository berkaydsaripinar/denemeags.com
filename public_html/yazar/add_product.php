...
$product_upload_dir = PRODUCT_UPLOADS_DIR . '/';
$video_upload_dir = VIDEO_UPLOADS_DIR . '/';
$image_upload_dir = PUBLIC_PRODUCT_UPLOADS_DIR . '/';

// Klasör kontrolü
ensure_directory($product_upload_dir);
ensure_directory($video_upload_dir);
ensure_directory($image_upload_dir);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'])) {
        ...
        if (isset($_FILES['resim_file']) && $_FILES['resim_file']['error'] === UPLOAD_ERR_OK) {
            ...
            move_uploaded_file($_FILES['resim_file']['tmp_name'], $image_upload_dir . $resim_url);
        }
